...
double logcond_Xi (const uword &Xi_i, const uword &caso, const uword &curlab, const uword &newlab, const rowvec &u_new, const uword &ip, const uvec &Ij, const double &K, const double &vt, const double &sigsq, const vec &Beta, mat U, uvec Xi, const uvec &Y)
{
...
void sample_Xi (const uvec &Ij, const double &K, const double &vt, const double &sigsq, const vec &Beta, mat &U, uvec &Xi, const uvec &Y, const mat &Simat)
{
     ...
     for (ip = 0; ip < accu(Ij); ++ip) {
          ...
          r = exp( logcond_Xi(prolab, caso, curlab, newlab, u_new, ip, Ij, K, vt, sigsq, Beta, U, Xi, Y) - logcond_Xi(curlab, caso, curlab, newlab, u_new, ip, Ij, K, vt, sigsq, Beta, U, Xi, Y) );
          // Set new label
...
//[[Rcpp::export]]
void MCMC (uvec Xi, const double &a_vt, const double &b_vt, const uvec &Ij, const double &K, const uvec &Y, const mat &Simat, const uword &nburn, const uword &nsams, const uword &nskip, const uword &ndisp, string path)
{
     // some quantities
     double I = accu(Ij);
     double J = Ij.n_elem;
     ...
     for (s = 1; s <= S; ++s) {
          // update parameters
          sample_Xi    (Ij, K, vt, sigsq, Beta, U, Xi, Y, Simat);
